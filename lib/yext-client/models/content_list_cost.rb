...
module YextClient
  ...
  class ContentListCost
    attr_accessor :type

    # Price in USD , e.g., 9.50. Must be >= 0.0 and <= 1000000.00
    attr_accessor :price

    # e.g., Per Gallon, Each
    attr_accessor :unit

    # Specified only if <b>type</b> is RANGE. In that case, this Cost represents a price range from <b>price</b> to <b>rangeTo</b>. Must be > <b>price</b> and <= 1000000.00
    attr_accessor :range_to

    # Specified only if <b>type</b> is OTHER. User-entered text, e.g., Market Price
    attr_accessor :other
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('String', ["PRICE", "RANGE", "NONE", "OTHER"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["PRICE", "RANGE", "NONE", "OTHER"])
    ...
    def hash
      [type, price, unit, range_to, other, options].hash
    end
